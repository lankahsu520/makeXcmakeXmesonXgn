...
LIBNAME_A =
LIBNAME_SO = helloworld
LIBNAME_MOD =

HELLOWORLD_VERSION="0x$(shell printf "%02X" $(VERSION_MAJOR))$(shell printf "%03X" $(VERSION_MINOR))$(shell printf "%03X" $(VERSION_REVISION))"
...
distclean: clean
	...
	$(PJ_SH_RMDIR) build_xxx $(SDK_ROOT_DIR) $(PJ_CPACK) .meson_config build.meson meson_options.txt
...
install: all
	...
	[ "$(LIBXXX_MOD)" = "" ] || $(PJ_SH_MKDIR) $(SDK_MODULES_DIR)
	@for sublib in $(LIBXXX_MOD); do \
		$(PJ_SH_CP) $$sublib* $(SDK_MODULES_DIR); \
		$(STRIP) $(SDK_MODULES_DIR)/`basename $$sublib.$(VERSION_FULL)`; \
	done
...
ifneq ("$(HOMEX_ROOT_DIR)", "")
	...
	[ "$(LIBXXX_MOD)" = "" ] || $(PJ_SH_MKDIR) $(HOMEX_MODULES_DIR)
	@for sublib in $(LIBXXX_MOD); do \
		$(PJ_SH_CP) $$sublib* $(HOMEX_MODULES_DIR); \
		$(STRIP) $(HOMEX_MODULES_DIR)/`basename $$sublib.$(VERSION_FULL)`; \
	done
